...
using Sale_Saas.Application.Features.BenefitHistoryFeature.Commands;
using Sale_Saas.Application.Features.BenefitHistoryFeature.Queries;
...
namespace Sale_Saas.API.Controllers
{
	...
	[ApiController]
	public class BenefitHistoryController : BaseController
	{
		private readonly ILoggerService _loggerService;

		public BenefitHistoryController(ILoggerService loggerService)
		{
			_loggerService = loggerService;
		}

		[HttpPost("get-list-with-pagination")]
        [HasPermission(PolicyTypes.Sale_QL.QL_HISTORY)]
        public async Task<IActionResult> GetListWithPagination([FromQuery] GetListWithPaginationQueryRequest request)
		{
			try
			{
				...
				return Ok(await Mediator.Send(new BenefitHistory_GetListWithPaginationQuery(userId, request)));
			}
			catch (Exception ex)
			{
		...
		public async Task<IActionResult> GetById(Guid id)
		{
			try
			{
                ...
                return Ok(await Mediator.Send(new BenefitHistory_GetByIdQuery(userId, id)));
			}
			catch (Exception ex)
			{
                _loggerService.WriteErrorLog(ex, functionName: MethodBase.GetCurrentMethod()?.Name ?? "UnknownMethod");
		...
		public async Task<IActionResult> AddOrUpdate([FromBody] List<AddOrUpdateRequest> request)
		{
			try
			{
				...
				return Ok(await Mediator.Send(new BenefitHistory_AddOrUpdateCommand(userId, request)));
			}
			catch (Exception ex)
			{
                _loggerService.WriteErrorLog(ex, functionName: MethodBase.GetCurrentMethod()?.Name ?? "UnknownMethod");
		...
		public async Task<IActionResult> DeleteByIds(string ids, Guid ApplicationUserId)
		{
			try
			{
                ...
                return Ok(await Mediator.Send(new BenefitHistory_DeleteByIdCommand(userId, request)));
			}
			catch (Exception ex)
			{
                _loggerService.WriteErrorLog(ex, functionName: MethodBase.GetCurrentMethod()?.Name ?? "UnknownMethod");
