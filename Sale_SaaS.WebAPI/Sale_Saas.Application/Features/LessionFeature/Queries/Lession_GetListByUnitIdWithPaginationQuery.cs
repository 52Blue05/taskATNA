...
public record Lession_GetListByUnitIdWithPaginationQuery(Guid UnitId, GetListWithPaginationQueryRequest RequestData) : IRequest<Result<PaginatedList<LessionDto>>>;

public class Lession_GetListByUnitIdWithPaginationQueryHandler : IRequestHandler<Lession_GetListByUnitIdWithPaginationQuery, Result<PaginatedList<LessionDto>>>
{
    private readonly IApplicationDbContext _context;
    private IEventLogService _eventLogService;

    public Lession_GetListByUnitIdWithPaginationQueryHandler(IApplicationDbContext context, IEventLogService eventLogService)
    {
        _context = context;
        _eventLogService = eventLogService;
    }

    public async Task<Result<PaginatedList<LessionDto>>> Handle(Lession_GetListByUnitIdWithPaginationQuery request, CancellationToken cancellationToken)
    {
