...
public record UnitQuestion_GetListByUnitIdQuery(Guid UnitId) : IRequest<Result<IEnumerable<UnitQuestionDto>>>;

public class UnitQuestion_GetListByUnitIdQueryHandler : IRequestHandler<UnitQuestion_GetListByUnitIdQuery, Result<IEnumerable<UnitQuestionDto>>>
{
    private readonly IApplicationDbContext _context;
    private IEventLogService _eventLogService;

    public UnitQuestion_GetListByUnitIdQueryHandler(IApplicationDbContext context, IEventLogService eventLogService)
    {
        _context = context;
        _eventLogService = eventLogService;
    }

    public async Task<Result<IEnumerable<UnitQuestionDto>>> Handle(UnitQuestion_GetListByUnitIdQuery request, CancellationToken cancellationToken)
    {
        var query = from unitQuestion in _context.UnitQuestions
                    ...
                    where unitQuestion.DeleteFlag != true && unitQuestion.UnitId == request.UnitId
        ...
        var eventLog = await _eventLogService.Create("UnitQuestionFeature", "UnitQuestionFeature", "UnitQuestion_GetListByUnitIdQuery", Guid.Empty);
