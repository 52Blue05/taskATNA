...
public record BenefitMobile_AddV3Command(Guid userId, BenefitMobileAddV3Request RequestData) : IRequest<Result<BenefitDto>>;

public class BenefitMobile_AddV3CommandHandler : IRequestHandler<BenefitMobile_AddV3Command, Result<BenefitDto>>
{
    ...
    public BenefitMobile_AddV3CommandHandler(IMapper mapper,
    ...
    public async Task<Result<BenefitDto>> Handle(BenefitMobile_AddV3Command request, CancellationToken cancellationToken)
    {
        ...
        CheckDuplicateGoal(benefit.TargetFluctuations);
        ...
        if (isManager && request.RequestData.RolePositionId != RolePositionEnum.EMPLOYEE.ToString())
        {
