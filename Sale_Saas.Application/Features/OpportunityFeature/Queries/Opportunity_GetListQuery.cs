...
{
    public record Opportunity_GetListQuery(FilterQueryRequest RequestData) : IRequest<Result<IEnumerable<OpportunityDto>>>;

    public class Opportunity_GetListQueryHandler : IRequestHandler<Opportunity_GetListQuery, Result<IEnumerable<OpportunityDto>>>
    {
        private readonly IApplicationDbContext _context;
        private readonly IMapper _mapper;

        public Opportunity_GetListQueryHandler(IMapper mapper, IApplicationDbContext context)
        {
            _context = context;
            _mapper = mapper;
        }

        public async Task<Result<IEnumerable<OpportunityDto>>> Handle(Opportunity_GetListQuery request, CancellationToken cancellationToken)
        {
            var query = (from cv in _context.Opportunities
            ...
            {
                query = query.Where(x => x.CustomerName.Contains(request.RequestData.TextSearch) ||
                                         x.TechnicalLead.Contains(request.RequestData.TextSearch) ||
                                         x.ApplicationUser.FullName.Contains(request.RequestData.TextSearch));
            }

            if (!string.IsNullOrEmpty(request.RequestData.Code))
            {
                //query = query.Where(x => x.Accountable.Code.Contains(request.RequestData.Code));
            }
            ...
            return Result<IEnumerable<OpportunityDto>>.Success(data);
        }
    }
}
