...
{
    public record Opportunity_GetByIdQuery(Guid Id) : IRequest<Result<OpportunityDto>>;
    public class Opportunity_GetByIdQueryHandler : IRequestHandler<Opportunity_GetByIdQuery, Result<OpportunityDto>>
    {
        private readonly IApplicationDbContext _context;
        private readonly IMapper _mapper;

        public Opportunity_GetByIdQueryHandler(IMapper mapper, IApplicationDbContext context)
        {
            _context = context;
            _mapper = mapper;
        }

        public async Task<Result<OpportunityDto>> Handle(Opportunity_GetByIdQuery request, CancellationToken cancellationToken)
        {
            //OpportunityDto? Opportunity = await (from cv in _context.Opportunities
            ...
            var opportunity = await OpportunityService.GetOpportunity(request.Id, _context);
