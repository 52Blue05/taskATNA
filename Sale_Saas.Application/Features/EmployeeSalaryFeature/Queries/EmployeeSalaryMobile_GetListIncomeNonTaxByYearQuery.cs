...
public record EmployeeSalaryMobile_GetListIncomeNonTaxByYearQuery(Guid UserId, int Year) : IRequest<Result<IncomeNonTaxDetailByYearDto>>;
public class EmployeeSalaryMobile_GetListIncomeNonTaxByYearQueryHandler : IRequestHandler<EmployeeSalaryMobile_GetListIncomeNonTaxByYearQuery, Result<IncomeNonTaxDetailByYearDto>>
{
    ...
    public EmployeeSalaryMobile_GetListIncomeNonTaxByYearQueryHandler(IMapper mapper, IApplicationDbContext context, IApplicationRoleService roleService, IEventLogService eventLogService, ITenantApplicationService tenantService, IApplicationUserService applicationUserService)
    {
        _context = context;
        _mapper = mapper;
    ...
    public async Task<Result<IncomeNonTaxDetailByYearDto>> Handle(EmployeeSalaryMobile_GetListIncomeNonTaxByYearQuery request, CancellationToken cancellationToken)
    {
        ...
        var listIncomeNonTaxByYear = new IncomeNonTaxDetailByYearDto()
        {
            nam = request.Year.ToString(),
            cacKhoanThuNhapKhongChiuThue = listIncomeByMonthAndYearUpdated.Sum(s => s.IncomeNonTax).ToString(),
            chiTietTheoThang = new List<IncomeNonTaxDetailByMonthDto>()
        };

        foreach (var item in listIncomeByMonthAndYearUpdated)
        {
            listIncomeNonTaxByYear.chiTietTheoThang.Add(new IncomeNonTaxDetailByMonthDto()
            {
                thang = item.Month.ToString(),
                thuNhap = item.IncomeNonTax.ToString()
            });
        }

        return Result<IncomeNonTaxDetailByYearDto>.Success(listIncomeNonTaxByYear);
    }
}
