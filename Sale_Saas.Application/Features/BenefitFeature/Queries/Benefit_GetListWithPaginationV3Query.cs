...
public record Benefit_GetListWithPaginationV3Query(Guid userId, GetListWithPaginationQueryRequest RequestData) : IRequest<Result<PaginatedList<BenefitDto>>>;

public class Benefit_GetListWithPaginationV3QueryHandler : IRequestHandler<Benefit_GetListWithPaginationV3Query, Result<PaginatedList<BenefitDto>>>
{
    ...
    public Benefit_GetListWithPaginationV3QueryHandler(
    ...
    public async Task<Result<PaginatedList<BenefitDto>>> Handle(Benefit_GetListWithPaginationV3Query request, CancellationToken cancellationToken)
    {
        ...
        if (request.RequestData.RoleType != null)
        {
            ...
            if (request.RequestData.RoleType == RoleType.MYSELF.ToString())
            {
                ...
                query = query.Where(s => s.ApplicationUser.Id == request.RequestData.UserId
                                      && s.ApplicationRoleId.HasValue
                                      && listRoleOfUser.Contains(s.ApplicationRoleId.Value));
            }
        ...
        var eventLog = await _eventLogService.Create("BenefitFeature", "BenefitFeature", "Benefit_GetListWithPaginationV3Query", request.userId);
